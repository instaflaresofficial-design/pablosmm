...
const outputPath = path.resolve(__dirname, "../../lib/admin/preferences/theme.ts");

const files = fs.readdirSync(presetDir).filter((file) => file.endsWith(".less"));
...
const presets = files.map((file) => {
  ...
  const label = labelMatch?.[1]?.trim() ?? file.replace(".less", "");
  const value = valueMatch?.[1]?.trim() ?? file.replace(".less", "");
...
const globalStylesPath = path.resolve(__dirname, "../../app/admin/admin.css");
...
async function main() {
  ...
  const formatted = execFileSync(npxCmd, ["@biomejs/biome", "format", "--stdin-file-path", outputPath], {
    input: updated,
    encoding: "utf8",
  });
