...
import { ProviderVerificationPortal, ServiceItem } from "@/app/admin/providers/verify/_components/provider-verification-portal";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${provider.name} — Service Verification Portal`,
    description: `Verify and correct service data for ${provider.name}.`,
...
async function getServices(): Promise<ServiceItem[]> {
  try {
    ...
    console.error("Provider portal: backend fetch error:", error);
    return [];
  }
}

export default async function ProviderPortalPage({ params }: PageProps) {
  const { token } = await params;
  const provider = getProviderByToken(token);
  ...
  // Strictly filter services belonging to this provider only
