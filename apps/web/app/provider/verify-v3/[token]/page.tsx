...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${provider.name} — Working Services Picker (V3)`,
...
async function getServices(): Promise<ServiceItem[]> {
  try {
    ...
    const res = await fetch(`${baseUrl}/provider/services`, {
      cache: "no-store",
    });
    if (!res.ok) throw new Error("Failed to fetch services");
    const data = await res.json();
    
    // Safety check: if API returns an object (e.g. legacy cache) instead of an array
    if (data && !Array.isArray(data) && data.services && Array.isArray(data.services)) {
      return data.services;
    }
    
    return Array.isArray(data) ? data : [];
  } catch (error) {
    console.error("Provider portal v3: backend fetch error:", error);
    return [];
  }
}

export default async function ProviderPortalV3Page({ params }: PageProps) {
  const { token } = await params;
  const provider = getProviderByToken(token);
