...
func (s *Service) Fetch(targetURL string) (*Metadata, error) {
	...
	if oEmbedMeta != nil {
		if oEmbedMeta.Title != "" { m.Title = oEmbedMeta.Title }
		if oEmbedMeta.Image != "" { m.Image = oEmbedMeta.Image }
	}
	...
	if strings.Contains(targetURL, "instagram.com") {
		...
		m.Followers, m.Following, m.Posts, m.Likes, m.Comments, m.Views = parseInstagramStats(desc)
...
func parseInstagramStats(desc string) (followers, following, posts, likes, comments, views int) {
	...
	cRe := regexp.MustCompile(`(?i)([\d,.]+)([KMB]?)\s*Comments`)
	vRe := regexp.MustCompile(`(?i)([\d,.]+)([KMB]?)\s*Views`)
	...
	if match := lRe.FindStringSubmatch(desc); len(match) >= 3 {
		likes = parseCount(match[1], match[2])
	}
	if match := cRe.FindStringSubmatch(desc); len(match) >= 3 {
		comments = parseCount(match[1], match[2])
	}
	if match := vRe.FindStringSubmatch(desc); len(match) >= 3 {
		views = parseCount(match[1], match[2])
	}
	return
}
